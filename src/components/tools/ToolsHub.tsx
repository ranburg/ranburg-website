...
export default function ToolsHub() {
  ...
  const filtered = (() => {
    ...
    return activeCategory === "all" ? TOOLS_CONFIG : TOOLS_CONFIG.filter((t) => t.category === activeCategory);
  })();

  return (
    <div className="space-y-8">
      <ToolSearch
        placeholder="Search by tool name, category, or keyword (e.g. json, finance, salesforce)…"
        showResults={false}
        className="max-w-xl"
        value={searchQuery}
      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        ...
        <div>
                    ...
                    <Link href={`/tools/${tool.slug}`} className="group block h-full">
                      <div className="glass-card relative h-full overflow-hidden p-6 transition-all group-hover:border-accent/30">
                        ...
                        <div className="relative">
                          ...
                          <span className="text-xs font-semibold uppercase tracking-wider text-accent">{tool.badge}</span>
                          <h3 className="mt-2 text-lg font-bold text-theme-heading transition-colors group-hover:text-accent">{tool.title}</h3>
                          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-theme-muted">{tool.shortDescription}</p>
          ...
          {filtered.length === 0 && (
            <p className="py-12 text-center text-slate-500">No tools match your search. Try &ldquo;json&rdquo;, &ldquo;finance&rdquo;, or &ldquo;salesforce&rdquo;.</p>
          )}
        </div>
      </div>
    </div>
